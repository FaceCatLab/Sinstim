function sinstim1p8p9_customStimList_win64(ParamFile,ImageDir,SaveDir,SubjectNameArg,isBlockMode)
...
%% for windows64 added by Giulia for BPN computer
global ioObj LPT1
...
%%% START CJ modifications
% The idea is to load a file that contains the name of stimuli to be
% displayed in the order with which they should be displayed. Add a field
% to the ByPart structure with the name of the file to be loaded.
if isfield(ByPart,'StimList');
    %first check if there are multiple parts. For now I will only make this
    %work if there is one stimulus.
    if length(ByPart.StimList) > 1 || nParts > 1
        error('providing a list of stimuli currently only work with 1 stimulus');
    end
    
    %load the text/mat file
    %1.make sure we have a filesep to imagedir
    ImageDirList = ImageDir;
    if ImageDirList(end) == filesep;
        ImageDirList(end) = [];
    end
    ImageDirList(end+1) = filesep;
    
    %load based on extension
    if iscell(ByPart.StimList)
    ByPart.StimList = ByPart.StimList{1};
    end
    [ListPathstr, ListName, ListExt] = fileparts(ByPart.StimList);
    if strcmpi(ListExt,'');%if no extension is provided try 2 things.
        try
            StimList = importdata([ImageDirList,ByPart.StimList,'.txt']);
        catch
            StimList = importdata([ImageDirList,ByPart.StimList,'.mat']);
        end
    elseif strcmpi(ListExt,'.mat') || strcmpi(ListExt,'.txt');
        StimList = importdata([ImageDirList,ByPart.StimList]);
    end
    
    ByPart.stimSeq = StimList;
    
    %determine the location of each stimulus in the stimList within the list of
    %stimuli loaded by sinstim (Files).
    StimListOrder = [];
    for ss = 1:length(StimList);
        %remove potential " that get added in text
        %files sometimes
        currentStim = lower(strrep(StimList{ss},'"',''));
        %find image location in Files List. There
        %should always be 1 instance of each file
        %in FILES.
        StimListOrder(ss) = find(strcmp(currentStim,Files));
    end
    % change ByPart.RandPattern to be the sequence
    ByPart.RandPattern{1} = StimListOrder;
end

%%%% END CJ modifications to main sinstim function
...
%    openparallelport_inpout32(hex2dec('d010'));
    ...
    %% for windows 64 added by Giulia
    ioObj=io32;% io64;%create a parallel port handle
    status=io32(ioObj);%io64(ioObj);%if this returns '0' the port driver is loaded & ready
    assert(status==0, 'ERROR: PARALLEL PORT !');
    LPT1=hex2dec('0378');% instead of iLPT1= hex2dec('D010'); % instead of openparallelport_inpout32(hex2dec('D010'));
...
if length(Buff1) == length(Buff2) && all(Buff1 == Buff2) % Buff, Buff
    ...
    if ischar(RandPattern);
        %regular case where RandPattern refers to stimlus randomiaztion
        %sequence.
        order = randpattern(1:nImages,RandPattern,nHalfPhases); %  ..replace randele by randpattern>
    else
        %case where RandPattern is actually the order of stimuli to be
        %presented.        
        while length(RandPattern) < nHalfPhases
            RandPattern = [RandPattern RandPattern];
        end
        order = RandPattern(1:nHalfPhases);
    end
    %CJ EDIT ends
...
elseif all(isnan(Buff1)) && length(Buff2) > 1
    ...
    if ischar(RandPattern);
        ...
        %case where RandPattern is actually the order of stimuli to be
        %presented.
         while length(RandPattern) < nPhases
            RandPattern = [RandPattern RandPattern];
        end
        order = RandPattern(1:nPhases);
    end
    %CJ EDIT ends
...
if Send == 'P'
    ...
    if ~isabort 
        if length(BytesToSend) > N
            ...
            %Suddha changed this line bc of timing issues at PBN lab, EEG
            %room left computer. Using a wait of (1) prevented the triggers
            %from showing up in the eeg viewer
            %wait(1);
            wait(5);
            sendparallelbyte(0);
        else
