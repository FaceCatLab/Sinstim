function sinstim1p8p9_win64(ParamFile,ImageDir,SaveDir,SubjectNameArg,isBlockMode)
...
for p = 1 : nParts
    ...
    if Gen.Background.isBackground
        ...
        Images(p).MatricesBG = backgroundmask(Images(p).MatricesI,...  <-- CRITICAL STEP !
            Gen.Background.OriginalColor,bg.ColorError{p},bg.ErodeBorder{p},bg.FillHoles{p});    else
...
%% for windows 64
    ...
    assert(status==0, 'ERROR: PARALLEL PORT NOT OPEN!');
