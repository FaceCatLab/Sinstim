...
%    returns code for the current keyboard library. (See GETCOSYLIB('Keyboard').)
%
...
% Ben,  Oct. 2008   v1
%       Jul. 2009   v2 (CosyGraphics 2-beta7)
...
elseif ischar(key)		% NAME -> NUM + CODE
    ...
    i = strmatch(pattern,COSY_KEYBOARD.TABLES.lowercasenames);
    keyNumber  = COSY_KEYBOARD.TABLES.(lib).KeyNumbers(i,1);
    keyNumber2 = COSY_KEYBOARD.TABLES.(lib).KeyNumbers(i,2);
    keyCode(keyNumber) = 1;
...
if nargout > 1 % <v2-beta10, backported on 2-beta9/7dec>
    ...
    if islogical(key) || isnumeric(key)  % CODE|NUM -> NUM + CODE
        ...
        for k = 1 : length(keyNumber)
            for j = 1:2
                f = find((COSY_KEYBOARD.TABLES.(lib).KeyNumbers(:,j) == keyNumber(k)) & ...
                    COSY_KEYBOARD.TABLES.(lib).KeyNumbers(:,3-j));
                if ~isempty(f)
                    n2 = COSY_KEYBOARD.TABLES.(lib).KeyNumbers(f(1),3-j);
                    keyNumber2(k) = n2;
                    keyCode2(n2) = 1;
                end
            end
