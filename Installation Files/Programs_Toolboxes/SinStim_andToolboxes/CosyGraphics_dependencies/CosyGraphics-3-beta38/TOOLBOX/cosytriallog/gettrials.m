...
% GETTRIALS  Get all trial data accumulated from first STARTTRIAL call.
...
%% MAIN
tmpfile = fullfile(cosydir('tmp'),'trials.mat');
s = load(tmpfile);
tr = s.Trials;

if ~nargin || ~strcmpi(varargin{1},'-raw')
    tr = sub_processtrials(tr);
end


%% SUB_FUN: PROCESSTRIALS
function tr = sub_processtrials(tr)
% COSYHELPER_PROCESSTRIALS  Post-processing on trials data. Used by GETTRIALS.
% TR = SUB_PROCESSTRIALS(TR)
...
for i = 1 : nTrials
    ...
    start  = tr(i).PERDISPLAY.ScreenFlipReturnedTimeStamps(1:end-1);
    finish = tr(i).PERDISPLAY.ScreenFlipCalledTimeStamps(2:end);
    ...
    % Errors
    tr(i).ERRORS_MissedFrames = nansum(tr(i).PERDISPLAY.MissedFramesPerDisplay);
    if tr(i).isParallelOut
...
%%  Convert double -> single/int   <Suppressed: Caused to much pb in M6.5. TODO: Reuse it when support for M6.5 will be dropped>
