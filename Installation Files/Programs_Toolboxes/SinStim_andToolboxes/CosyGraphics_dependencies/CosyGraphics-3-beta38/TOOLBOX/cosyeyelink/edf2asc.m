...
% As a workaround, we'll copy EDF2ASC.EXE into "X:\cosygraphics\lib\" and *.EDF files in "X:\cosygraphics\tmp\",   
...
% edf2asc.m is a part of CosyGraphics and therefore uses the CosyGraphics's std "X:\cosygraphics\*" dirs, but it should be 
...
%   - Avoid using other BenTools or CosyGraphics functions (e.g.: cosydir). <!!!>
...
% Make the "X:\cosygraphics\lib" and "X:\cosygraphics\tmp\" dirs, if not already existing: <Don't use checkdir, see above>
MFileFullName = which(mfilename);
drive = MFileFullName(1:2);
TmpDir = [drive filesep 'cosygraphics' filesep 'tmp' filesep]; % <in case of modification, change also the mkdir args, just below>
LibDir = [drive filesep 'cosygraphics' filesep 'lib' filesep];
if ~exist([drive filesep 'cosygraphics'])
    [success,msg] = mkdir(drive,'cosygraphics');
    if ~success, error(['Cannot create "' drive filesep 'cosygraphics' filesep '".' 10 msg]), end
end
if ~exist(TmpDir)
    [success,msg] = mkdir([drive filesep 'cosygraphics'],'tmp');
    if ~success, error(['Cannot create "' TmpDir '".' 10 msg]), end
end

%% Copy files to "X:\cosygraphics\lib" and "X:\cosygraphics\tmp"
% Copy EDF2ASC.EXE into "X:\cosygraphics\lib\" dir, if not already done:
...
% Copy *.EDF file into "X:\cosygraphics\tmp\" dir:
...
%% Delete all *.EDF and *.ASC files from "X:\cosygraphics\tmp\"
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%% SUB-FUCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%% %%
%% dispinfo <copy of CosyGraphics's function>
...
%    'WARNING' or 'ERROR'.
%
%    This function is intended to be used by all CosyGraphics fuunctions to display their message in the
