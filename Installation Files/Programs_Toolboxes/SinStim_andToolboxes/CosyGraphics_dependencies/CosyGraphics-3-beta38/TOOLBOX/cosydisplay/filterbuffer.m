...
% FILTERBUFFER  Filter an offscreen buffer. (PTB only) <NOT YET OPTIMIZED: DON'T USE!!!>
...
bf = Screen('TransformTexture', b, convoperator, []); % 1.4 ms on DARTAGNAN
% t4=time; dt4=t4-t3

% Global vars
COSY_DISPLAY.BUFFERS.OffscreenBuffers(end+1) = bf;
COSY_DISPLAY.BUFFERS.isTexture(end+1) = 1;
ib = find(COSY_DISPLAY.BUFFERS.OffscreenBuffers == b);
COSY_DISPLAY.BUFFERS.isFloat(end+1) = COSY_DISPLAY.BUFFERS.isFloat(ib)