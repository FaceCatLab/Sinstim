...
% WAITSYNCH  Wait function for real-time display programs. <DEPRECATED: Give duration as argument to DISPLAYBUFFER.>
...
% 2.0   Nov 2008  Major fix (!): Substract COSY_DISPLAY.LastDisplayTimeStamp.
%                 Check abort.
% 2.1   Jan 2010  


global COSY_DISPLAY COSY_TRIALLOG
...
if ~nargin ...  %    t0 = WAITSYNCH
    ...
    if nargin < 3 % it's not  WAITSYNCH(t,'ms','-displaybuffer')  which is used by displaybuffer.
        if COSY_TRIALLOG.isStarted % If we are between STARTTRIAL and STOPTRIAL..
...
%                 end
    ...
    if isempty(COSY_DISPLAY.LastDisplayTimeStamp)
%         error('WAITSYNCH can only be called between two DISPLAYBUFFER calls.')
    end
    t0 = time;
    F0 = floor( (t0 - COSY_DISPLAY.LastDisplayTimeStamp) / OneFrame );
