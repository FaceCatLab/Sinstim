function [DisplayTimes,FramesMissed,varargout] = displaybuffersequence(varargin)
...
% v3.1   Mar  2013  3-beta37        Fix CGr 3 port: Fix draw() args.
...
%                                   Fix background color after splash.
% v4.0   Mar  2013  3-beta37        Split


global COSY_FUNCTIONS
persistent B X Y Durations ALPHA ETA THETA XSCALE YSCALE RECT3 Get Send ValuesToGet BytesToSend Draw FrameErrorEvent isPhotodiode isIsochronous Priority Forward
...
%% INPUT ARGS: Mode: Preset?, Display?, Preset+Display?
if nargin
    switch lower(varargin{1})
        case 'preset'
            dispinfo(mfilename,'info','Preparing sequence...')
            doPreset  = 1;
            doDisplay = 0;
            varargin(1) = [];
        ...
        otherwise
            if isnumeric(varargin{1})
                if all(isbuffer(varargin{1}))
                    doPreset  = 1;
                    doDisplay = 1;
                else
                    stopfullscreen;
                    error('Invalid input argument: Invalid buffer handle(s).')
                end
            else
                stopfullscreen;
                error('Unexpected input argument: First argument should be buffer handles.')
            end
    end
...
if doPreset
    ...
    dispinfo(mfilename,'info','Done. Sequence ready to be displayed.')
end
...
if doDisplay
    ...
    %% Main Loop
    dispinfo(mfilename,'info','Starting real-time display...')
    j = 1;
    ...
    clear B X Y Durations ALPHA ETA THETA XSCALE YSCALE RECT3 Get Send ValuesToGet BytesToSend Draw FrameErrorEvent isPhotodiode isIsochronous Priority Forward
