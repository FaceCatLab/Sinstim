...
% STOREIMAGE  Pre-store image matrix in offscreen buffer. {slow, except for small matrices}
%   b = STOREIMAGE(I)  creates an offscreen buffer, stores image I in it and
%   returns the buffer handle, b. I is an M-by-N-by-3 matrix in the range 0.0 
%   to 1.0 (as returned by LOADIMAGE), or a cell array (1D or 2D) of such matrices.
%
...
if iscog % CG
    ...
    if nargin == 1
        ...
        if strcmp(class(I),'uint8')
            ...
            if 1 % <v2-beta39> New way: use half float if I is double or single
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                isFloatPrecision = 1; % 1 = half float (16 bits), 2 = float (32 bits)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if max(I(:)) >= 2, I = I / 255; end
            else % Old way: never use float precision. Keep code in case of troubles.
                isFloatPrecision = 0; 
                if max(I(:)) <= 1, I = I * 255; end  % <faster when not using round()>
            end
        end
        b = Screen('MakeTexture',gcw,I,0,0,isFloatPrecision); % <TODO: floatprecision ?? , textureOrientation ??>
        Screen('PreloadTextures',gcw,b);  % <TODO: really needed ?>
        COSY_DISPLAY.BUFFERS.OffscreenBuffers(end+1) = b;
        COSY_DISPLAY.BUFFERS.isTexture(end+1) = 1;
