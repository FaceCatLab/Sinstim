...
%    SETPRIORITY OPTIMAL   raises priority, letting CosyGraphics choose for you the best compromize between 
    ...
    if ischar(priority)         
        ...
        if IsLinux % ('IsLinux' is a PTB function) Linux
            switch priority
                ...
                case 'REALTIME',                priority_num = 90; % (max is 99)
...
% %% Update CosyGraphics's global var
% COSY_GENERAL.CurrentPriority = priority; % <TODO: Fix case str/num on different OSes>  <Suppr. in 2-beta41>
...
if ~isSilent
    if ischar(priority)
        if ispc, % Windows (always waiting a Windows priority name as argument)
            if strcmp(priority_str,'OPTIMAL') % special case of "OPTIMAL" (this is not a MS name, it's aCosyGraphics name)
