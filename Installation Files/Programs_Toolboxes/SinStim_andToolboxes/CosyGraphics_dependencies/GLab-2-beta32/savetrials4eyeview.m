...
%% Global Vars
glabglobal  % GLab vars
global USER % Eyeview var

%% Input Args
% File name:
if nargin < 1
    error('''savefile'' argument is missing.')
end
if strcmpi(gdffilename,'GUI')
    if isopen('display')
        beep
        msg = {'GLab display was not stopped!'; ...
            [mfilename ' was to stop GLab to let you see the dialog box.']; ...
            ['Please move ' mfilename ' after stopglab in your script.'] };
        warning(msg);
        warndlg(msg);
        stopglab;
    end
...
%% FILE
FILE.Tsystem = 'GLab';
FILE.Tsystem_version = vnum2vstr(glabversion);
...
BLOCKS.protocol_file = GLAB_STATUS.UserScript;
d = datevec(date);
...
%% Get trials data
GLabTrials = gettrials;

%% GLabTrials -> TRIALS
for i = 1 : length(GLabTrials)
    t0 = GLabTrials(i).PERDISPLAY.TimeStamps(1);
    TRIALS(i).Tt = GLabTrials(i).PERDISPLAY.TimeStamps - t0;
    TRIALS(i).Et = GLabTrials(i).PERDISPLAY.TimeStamps - t0;
    TRIALS(i).rTh = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(1).XY(:,1));
    TRIALS(i).rTv = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(1).XY(:,2));
    for t = 2 : length(GLabTrials(i).TargetNames)
        if strcmpi(GLabTrials(i).TargetNames{t},'eye') % online eye..
            TRIALS(i).rOh = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(t).XY(:,1)); % store it in Oh, Ov
            TRIALS(i).rOv = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(t).XY(:,1));
        else
            TRIALS(i).(['rT' int2str(t) 'h']) = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(t).XY(:,1));
            TRIALS(i).(['rT' int2str(t) 'v']) = pix2deg(GLabTrials(i).PERDISPLAY.TARGETS(t).XY(:,2));
        end
    end
    
    % Eyeview events:
    T = GLabTrials(i).PERDISPLAY.Tag;
    % trial
    i0 = 1;
    i1 = size(T,1);
...
%% Add date suffix and .GDF (GLab Data File) extension to file name
...
dispinfo(mfilename,'info',['Saving GraphicsLab data to "' gdffilename '"...'])
...
if isfieldtrue(GLAB_EYELINK,'isFileTransfered')
    source = fullfile(glabtmp,'active.edf');
    ...
    dispinfo(mfilename,'warning','No EyeLink data file.')
end
