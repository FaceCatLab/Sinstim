...
try
    for i = 1:3 % <Hack!!! Do it 3 times because of current lack of reliability.>
        if     v <= '6'
            sendserialbytes(GLAB_PSB.SerialPortNumber, command); % <awfully slooooooow on Matlab 7.5>
        elseif v >= '7'
            fprintf(GLAB_PSB.SerialPortObject, command);
        end
    end
    
catch
    dispinfo(mfilename,'error',['Failed to send bytes through ' GLAB_PSB.SerialPortName ' port.']);
    return % <===!!!
