...
elseif ischar(key)
    if strcmpi(key,'?') | strcmpi(key,'all')                        % DISPLAY ALL NAMES
        disp(GLAB_KEYBOARD.TABLES.CamelCaseNames)
    else                                                            % NAME -> NAME
        keyName = key;
    end
    return % !!!
...
for k = 1 : numel(keyNamesCell)
    i1 = find(GLAB_KEYBOARD.TABLES.(lib).KeyNumbers(:,1) == keyNumber(k));
    i2 = find(GLAB_KEYBOARD.TABLES.(lib).KeyNumbers(:,2) == keyNumber(k));
    i = min([i1,i2]);
    if isempty(i)
        keyNamesCell{k} = '';
    else
        keyNamesCell{k} = GLAB_KEYBOARD.TABLES.CamelCaseNames(i,:);
