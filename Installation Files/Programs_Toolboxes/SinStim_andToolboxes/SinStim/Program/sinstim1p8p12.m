...
%   1.8.2   09 jun          2-beta32       §Fun replaced by sub-function <!!!>: displayanimation -> sub_displayanimation <!!!> (same than v2-beta32's fun)
%                                          Fix missing last // byte:  <Same fix in GLab v2-beta39>
%                                           (First // byte: 254/255 bug: cannot reproduce bug.)
%                                          §Sine wave start angle: becomes a param.
...
%   1.8.10  22 dec          unstable        Suppport different scales for different parts: Gen.Scale -> ByPartScale, Scale (vector) -> SCALE (matrix). <Buggy!>
%   1.8.11  27 fev 2012     unstable        Fix history log above: v1.8.0 and 1.8.2 about change displayanimation -> sub_* (was not consistent with archive m-files).
%                                           Fixes: Apply JBB's fixes (bug reports 25-01 & 10-02-2012.) <Applied in SinStim 1.8.11's sub-fun, in GLab/CGr 2-beta32 & 3-beta12.>
%                                           Version number: Now get it from mfilename() (was hard-coded).
%   1.8.12  27 fev 2012     2-beta32        Add 'ForwardEvents' option. <[subfun_]displayanimation modified in: v2-beta32/v3-beta12/SinStim-1.8.12>
%                                           [subfun_]displayanimation: Clip byte to send to 255. Record sent bytes in stats.PERDISPLAY.ValuesActuallySent_byte.
%                                           datesuffix->subfun_datesuffix.
%                                           Example file: Update doc for obsolete params (Gen.ResponseKey was obsolete since v1.5.3).
%           01 mar                          Fix fix case one scale <broken in v1.8.10>
%   ---
%
%   Benvenuto JACOB, Institute of NeuroSiences (IoNS), Universite Catholique de Louvain.
...
% SinStim version #: get it from mfilename() <v1.8.11>
name = mfilename;
f = find(ismember(name,'0':'9'));
SinStimVersion = name(f(1):f(end));
SinStimVersion(SinStimVersion=='p') = '.';
...
% v1.1 Parameters:
if ~isfield(Gen,'ResponseKey'), Gen.ResponseKey = 'Space'; end % <Obsolete since v1.5.3>
...
if ~isfield(Gen.Events,'doForwardKeyboardEvents'), Gen.Events.doForwardKeyboardEvents = 0; end % <v1.8.12>
...
%% PREPARE ANIMATION:
    ...
    %   Change scale (Progr. note: Must be donne before "Delay before identity change")  <v1.3, rewritten v1.8.10 (buggy), fixed v1.8.12>
    ...
    if nParts>1 && Gen.AllPartsSharingScalesOf % Not-Independent scales mode (default): randomize once for all. <v1.8.10>
        p0 = Gen.AllPartsSharingScalesOf;
    else
        p0 = 1;
    end
    phases = [1, find(diff(PHASES(p0,:))) + 1, nFrames+1];
    if length(ByPart.Scale{p0}) == 1  % Constant scale
        scales0 = repmat(ByPart.Scale{p0}, 1, nFrames);
    else                              % Variable scale
        scales0 = randele(ByPart.Scale{p0},length(phases),1);
    end
    for p = 1 : nParts
...
%% EXPERIMENT!
    ...
    %--------------------------------------!!!--REAL-TIME--!!!------------------------------------
    [Times,FrameErr,KeyIDs,KeyPressTimes] = subfun_displayanimation(BUFFERS,X,XAlign,Y,YAlign,... <v1.8.3: became a sub-fun>
        'Alpha',ALPHA,...
        'AxesRotation',THETA,...
        'Scale',SCALE,...
    ...
    BytesActuallySent = stats.PERDISPLAY.ValuesActuallySent_byte; % <v2-beta32/v3-beta12/SinStim-1.8.12: NB: displayanimation() exports "stats" in caller space.>
    assignin('base','BytesActuallySent',BytesActuallySent);
...
% Save file & dir:
SaveFile = [SubjectName '_' ParamFile(1:end-2) subfun_datesuffix]; %<v1.8.12: datesuffix->subfun_datesuffix>
...
function [DisplayTimes,FramesMissed,varargout] = subfun_displayanimation(B,varargin)
...
% v2.8.1b/SinStim-1.8.3  Sep 2011 2-beta32   <backport v3-alpha1 into SinStim 1.8.3's sub-fun>
%                                            'Splash' option: Add DelayBuffer arg.
% v2.8.1c/SinStim-1.8.11 Feb 2012 2-beta32   Apply JBB's fixes (bug reports 25-01 & 10-02-2012.) <Applied in SinStim 1.8.11's sub-fun, in GLab/CGx 2-beta32 & 3-beta12.>
% v2.8.1d/SinStim-1.8.12 Feb 2012 2-beta32   <v2-beta32/v3-beta12/SinStim-1.8.12> Add 'Forward' option. Fix: 
%                                                                                 Clip output byte (b) to 255. Record BytesSent.
tic
...
BytesSent = NaN + zeros(1,N+1); %<v2-beta32/v3-beta12/SinStim-1.8.12>
...
% if j==1, t435=time-t0, end
    ...
    if     Send == 'P'
        ...
        if Forward.doIt && Forward.NextVal %<v2-beta32/v3-beta12/SinStim-1.8.12>
            b = b + Forward.NextVal;
        end
        b = min([b; 255]); % Clip b to 255 <v2-beta32/v3-beta12/SinStim-1.8.12>
        sendparallelbyte(b);
        if BytesToSend(j)
            SendTimes(j) = time;
        end
        BytesSent(j) = b; %<v2-beta32/v3-beta12/SinStim-1.8.12>
    elseif Send == 'S'
        if ~isnan(BytesToSend(j)) % <TODO: not updated>
...
% if j==1, t454=time-t0, end
    ...
    if Get == 'K'
        ...
        if Forward.doIt %<v2-beta32/v3-beta12/SinStim-1.8.12>
            Forward.NextVal = 0; % reset it
            [n,c,t] = checkkeydown(ValuesToGet);
            val = n(1);
            if val
                if val ~= Forward.PrevVal
                    f = find(Forward.InputValues==val);
                    if ~isempty(f)
                        Forward.NextVal = Forward.OutputValues(f);
                    end
                end
            end
            Forward.PrevVal = val;
...
if Send == 'P'
    ...
    if ~isabort 
        if length(BytesToSend) > N
            ...
            BytesSent(N+1) = BytesToSend(N+1); %<v2-beta32/v3-beta12/SinStim-1.8.12>
            wait(1);
            sendparallelbyte(0);
        else
...
%     pd.SentValues_byte = zeros(size(SendTimes)) + NaN;
    pd.ValuesActuallySent_byte = BytesSent; %<v2-beta32/v3-beta12/SinStim-1.8.12>
