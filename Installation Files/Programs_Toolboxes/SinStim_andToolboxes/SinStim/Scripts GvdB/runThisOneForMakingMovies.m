...
subjectnm = char(inputdlg('give movies name', '', 1, {'avi'}));
...
% maxNumOfVideos = cell2num(inputdlg('max number of videos without cleaning memory', '', 1, {'20'}));
maxNumOfVideos = 20;



% maxNumOfVideos = 10; % 20;
...
if ~exist(SaveDir)
    ...
    try
        [s, ok] = listdlg('PromptString','Select experiment design:','ListString',char(str2)); % create a list dialog that contains the name that where in the directory
        if ok ==1
...
if strcmp(runSinstimNow,'Yes')
    ...
    while runNr <= size(trialList, 1)
        ...
        if runNr/maxNumOfVideos == round(runNr/maxNumOfVideos)
            stoppsych;
        end
        sinstim_forMovie([expPath, '\parameters\',trialList{runNr,2}],[expPath, '\stimuli\',trialList{runNr,1}],SaveDir,[subjectnm, '_', trialList{runNr,1}],1);
