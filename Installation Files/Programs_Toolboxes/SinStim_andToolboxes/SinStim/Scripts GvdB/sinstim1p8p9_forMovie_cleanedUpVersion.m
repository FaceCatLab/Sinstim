function sinstim1p8p9_forMovie(ParamFile,ImageDir,SaveDir,SubjectNameArg,isBlockMode)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% changed this part for video         %%%%  
%%% changed Program/ by  sinstimForMoviesGoedele/
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
% Variable file
VarFile = [SinStimRoot 'sinstimForMoviesGoedele/sinstim_var.txt']; % <v1.7: Replace: save/load('sinstim_var.mat')  by: writetxt/savetxt('sinstim_var.txt')>
...
% addpath([SinStimRoot 'sinstimForMoviesGoedele/']);
% addpath([SinStimRoot 'sinstimForMoviesGoedele/Examples/']);
% addpath([SinStimRoot 'sinstimForMoviesGoedele/Test/']);
% savepath;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% changed until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% deleted from here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% %  writetxt(VarFile,c);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% deleted until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% added from here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Gen.Screen = 0;  % always display in window, never fullscreen
Gen.isIsochronousMode = 0; % don't correct missed frames (by skipping the next one). 
Gen.MissedFramesTolerance = 400000000000000;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% added until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Load Images
disp(' ')
disp([mfilename ': Loading images from BMP files...', ImageDir])
...
%% PREPARE ANIMATION:
    ...
    for p = 1 : nParts
        ...
        if ByPart.WhatVaries{p} == 'I' % && nImages > 1 <??? todo: Check this!>
            ...
            [B,A,Peaks,Phases, order] = subfun_preparesinanimation(Images(p).BuffersI,Images(p).BuffersI,Gen.Duration, ...
...
% for i = 1 : length(var), assignin('base',var{i},eval(var{i})); end
            [B,A,Peaks,Phases, order] = subfun_preparesinanimation(NaN,Images(p).BuffersI,Gen.Duration, ...
        ...
        elseif ByPart.WhatVaries{p} == 'L'
            ...
            [B,A,Peaks,Phases, order] = subfun_preparesinanimation(Images(p).BufferBG,Images(p).BufferI,Gen.Duration, ...
        ...
        elseif ByPart.WhatVaries{p} == 'T'
            ...
            [B,A,Peaks,Phases, order] = subfun_preparesinanimation([],Images(p).BufferI,Gen.Duration, ...
...
%     OldGamma = setgamma(Gen.ScreenGamma);
    stimdir = regexp(ImageDir, '\', 'split');
    stimdir = stimdir(end);
    try
    outputLine = [char(ParamFile(1:end-2)), ' ', char(stimdir), ' ', num2str(StimFreqs)];
    for stimNr = 1:numel(order)
        outputLine = [ outputLine,  ' ', Files{order(stimNr)} , ' ', num2str(scales(stimNr))];
    end
    dlmwrite([SaveDir 'movieInformation.txt'], outputLine, 'delimiter', '', 'newline', 'pc','-append');
    catch
    end
    
    movieFileNm = [SaveDir SubjectName '_' ParamFile(1:end-2) datesuffix];   %% added for video!!
    %-------------------------------------!!!--REAL-TIME--!!!-----------------------------------
    sub_displayanimation_forMovie(BUFFERS,X,XAlign,Y,YAlign, ... <v1.8.3: became a sub-fun> //
        'movieFileNm',   movieFileNm,...      % added movieFileNm for video!!
        'Alpha',ALPHA,...
        'AxesRotation',THETA,...
        'Scale',Scale,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% commented this part out for video %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% commented out until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [BUFFERS,ALPHA,Peaks,Phases, order] = subfun_preparesinanimation(Buff1,Buff2,Dur,SineFreq,AlphaRange,isHarmonic,RandPattern,GapBetweenPatterns,SineWaveStartAngle)
...
%% SUB_DISPLAYANIMATION_forMovie
function [DisplayTimes,FramesMissed,varargout] = sub_displayanimation_forMovie(B,varargin)
...
%%% added this part for video %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if strcmpi(varargin{i},'movieFileNm')
        movieFileNm = varargin{i+1};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% added until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        
    elseif strcmpi(varargin{i},'Get')
    ...
    elseif strcmpi(varargin{i},'Splash')
        ...
        if length(varargin) >= i+4 && isnumeric(varargin{i+4})
            ...
            Splash.TimeOut = inf;  % = max time to wait for keypress at start of trial
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	framesPerSec = 60; 
    frameInc = 1/framesPerSec;
	Priority = 'HIGH';	% moderate priority OK, just writing files

% 	movieFile = sprintf('%s_%s.avi', 'SSVEP_dump', datestr(now, 30));	% alas, no way to access block identifier in here...
	movieFile = [movieFileNm, '.avi'];	% alas, no way to access block identifier in here...
	map = colormap;
% 	compressMethod = 'none';	% Egad -- 3.7 GB for ONE friggin block?!?
	compressMethod = 'Cinepak';	% RLE or MSVC fail for truecolor images
% 	compressMethod = 'Indeo5';	% RLE or MSVC fail for truecolor images
	compressQuality = 100;	% higher is better. Default 75. Doesn't do anything?? <=100 blurry fix cross. boo
% 	moviePtr = avifile(movieFile, 'colormap', map, 'compression', compressMethod, 'fps', framesPerSec);
% 	moviePtr = avifile(movieFile, 'compression', compressMethod, 'quality', compressQuality, 'fps', framesPerSec);
	moviePtr = avifile(movieFile, 'colormap', map, 'compression', compressMethod, 'quality', compressQuality, 'fps', framesPerSec);
% 	moviePtr = avifile(movieFile, 'colormap', map, 'compression', compressMethod, 'fps', framesPerSec);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		waitsynch;	% wait until draw is finished (useful at all??)
		imageArray = Screen('GetImage', gcw);		% pull (exact?) screen image to array
		...
		% for Indeo compression, need to scale image values from 256 -> 236
		if ~isempty(strfind(compressMethod, 'Indeo')),
			imageArray = double(imageArray);
			imageArray = 236 * imageArray ./ 256;
			imageArray = uint8(round(imageArray));
		end
		imageFrame = im2frame(imageArray);

		moviePtr = addframe(moviePtr, imageFrame);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% added until here for video %%%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	moviePtr = close(moviePtr);	% dumbass code helper, of course I won't use it again I'm closing the damn object!
    varargout = {};
