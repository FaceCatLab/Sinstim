...
% BACKGROUNDMASK  Make a binary background mask. {slow}
...
%    bgcolor is a [r,g,b] vector in the range 0.0 to 1.0.
%
...
%
%    BG = BACKGROUNDMASK(I|C, <bgcolor,> err, Erode)  erodes the foreground form
%    (i.e.: remove pixels from the border). 'Erode' is the number of pixels of 
%    erosion. If it's a negative number, it's the background which will be eroded.
%
%    BG = BACKGROUNDMASK(I|C, <bgcolor,> err, Erode, FillHoles),  if FillHoles = 1,
%    fills the holes in the foreground shape if any. If FillHoles = 0, doesn't fill
%    the holes (default). <TODO!>
%
%    [BG, bgcolor] = BACKGROUNDMASK(I <,err>)  returns also the background color.
%    I must be a matrix, not a cell array.
%
% See also CHANGEBACKGROUNDCOLOR, ADDBACKGROUNDALPHA, HSVMASK.
%
% Ben,	Oct. 2007.
...
try   Dilate = varargin{2}; 
catch Dilate = 0;
end

% FillHoles
try   FillHoles = varargin{3}; 
catch FillHoles = 1;
end
