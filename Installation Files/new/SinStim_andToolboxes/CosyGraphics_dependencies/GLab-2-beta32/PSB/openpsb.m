...
% OPENPSB  Initialize Psychophysics Synchronisation Box (Benoit Gerard)
...
%% Global vars
GLAB_PSB.isPSB = 0;
GLAB_PSB.nBlankingLines = nBlankingLines;
GLAB_PSB.SerialPortNumber = SerialPortNumber;
GLAB_PSB.SerialPortName   = SerialPortName;
if isfilledfield(GLAB_PSB,'SerialPortObject'), GLAB_PSB.SerialPortObject = []; end
...
try
    ...
    elseif v >= '7'
        if ~isempty(GLAB_PSB.SerialPortObject) % If port has been already open..
            fclose(GLAB_PSB.SerialPortObject); % ..close it
            GLAB_PSB.SerialPortObject = [];    % ..and delete object
        end
        port = serial(SerialPortName,'baudrate',115200); % Open communication with PSB
        fopen(port);
        GLAB_PSB.SerialPortObject = port;
    end
    GLAB_PSB.isPSB = 1;
catch 
    disp(lasterr)
