function handle = LoadGLSLProgramFromFiles(filenames, debug, extraShaders, GLabInit)
% <LoadGLSLProgramFromFiles: Modified PTB 3.0.8 file. It overloads the eponymous 
%  PTB file. It avoids reading file on hd to load the shader, instead getting it
%  from the global var GLAB_DISPLAY.SHADERS.
%  It is called by opendisplay at startup to fill GLAB_DISPLAY.SHADERS. If new
%  shaders have to be used, opendisplay has to be updated.
% >
%
...
global GL;
global GLAB_DISPLAY %<ben>

%<ben>
if exist('GLabInit','var')
    GLAB_DISPLAY.ShaderFiles{end+1} = filenames;
elseif ischar(filenames)
    i = strmatch(filenames,GLAB_DISPLAY.ShaderFiles);
    if any(i)
        handle = GLAB_DISPLAY.ShaderHandles(i);
        return %                                                           <=== RETURN === !!!
    end
end
%</ben>
...
%<ben>
if exist('GLabInit','var')
    GLAB_DISPLAY.ShaderHandles(end+1) = handle;
end
%</ben>
