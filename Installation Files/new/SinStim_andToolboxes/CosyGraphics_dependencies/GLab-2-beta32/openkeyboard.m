function openkeyboard(isExclusive,TimeResolution,QueueLength)
...
if nargin < 2,
	...
	else            TimeResolution = 5;
	end
end
if nargin < 3,
	QueueLength = 100;
end
if nargin == 1 && strcmpi(isExclusive,'PTB')
    isPtbOnly = 1;
    isExclusive = 0;
else
    isPtbOnly = 0;
...
if iscog('Keyboard') && ~isPtbOnly
	...
	load([glabroot '/data.mat'],'CogInput_KeyMap_be')
...
% Execution time of "strmatch(name,GLAB_KEYBOARD.TABLES.lowercasenames)":
% 70 microseconds (on Matlab 6.5 on Athlon X2 4400+)

% For PTB: We need a filter to filter valid codes in 'keyCode' arg.
isValid = logical(zeros(1,255));
isValid(setdiff(unique(PTB(:)),0)) = 1;

% -> Global Vars
GLAB_KEYBOARD.TABLES.CamelCaseNames = NAMES;
GLAB_KEYBOARD.TABLES.lowercasenames = names;
GLAB_KEYBOARD.TABLES.COG.KeyNumbers = COG;
% GLAB_KEYBOARD.TABLES.COG.Filter = ;
GLAB_KEYBOARD.TABLES.PTB.KeyNumbers = PTB;
GLAB_KEYBOARD.TABLES.PTB.KeyCodeFilter = isValid;
