...
%% FIX POINT COLORS AND SHAPES
    ...
    if fp.NumChanges > 0
        ...
        r = r(1:nc);
        jj = [];
        [indShapeA,indShapeB,indColorA,indColorB]=fixatFunc(Gen.Duration,fp.ShapeChanges,fp.ColorChanges,fp.simultChanges,Gen.RefractoryStart,Gen.RefractoryEnd,getframedur,nFrames,fp.Refractory);
        ...
        switch fp.WhatChanges
            ...
            case 'B',   [indShapeA,indShapeB,indColorA,indColorB]=fixatFunc(Gen.Duration,fp.ShapeChanges,...
                        fp.ColorChanges,fp.simultChanges,Gen.RefractoryStart,Gen.RefractoryEnd,getframedur,nFrames,fp.Refractory);
                        saveChange(indColorA,indColorB,indShapeA,indShapeB,Gen.SaveDir)
                        for k = 1:3, FIXPOINTCOLOR(1,indColorA,k) = fp.ChangedColor(k); 
                        FIXPOINTCOLOR(1,indColorB,k) = fp.Color(k); end
                        FixPointShape(indShapeA) = repmat({fp.Shape},1,length(indShapeA));
                        FixPointShape(indShapeB) = repmat({fp.ChangedShape},1,length(indShapeB));
        end
