...
%   1.6.4   18 mar          2-beta14        Compress 'Images' structure :
%   makes file 5.3x lighter
...
%% PREPARE ANIMATION:
    ...
    %nFrames = length(sinewave(Gen.Duration,1,getscreenfreq)); % original (G.V. 24/06/13)
    nFrames = length(generatewave(Gen.Duration,1,getscreenfreq,'sine')); % sinewave -> generatewave (G.V. 24/06/13)
    BUFFERS = [];
    ALPHA   = [];
    PEAKS   = [];
    PHASES  = [];
    ...
    for p = 1 : nParts
        ...
        if ByPart.WhatVaries{p} == 'I' % && nImages > 1 <??? todo: Check this!>
            ...
            [B,A,Peaks,Phases] = subfun_preparesinanimation(Images(p).BuffersI,Images(p).BuffersI,Gen.Duration, ...
                ByPart.WaveFreq{p},ByPart.WaveForm{p},ByPart.AlphaRange{p},ByPart.isHarmonic{p},ByPart.RandPattern{p}); %v1.8.x: mode "I" not more supported for new features>
...
% for i = 1 : length(var), assignin('base',var{i},eval(var{i})); end
            [B,A,Peaks,Phases] = subfun_preparesinanimation(NaN,Images(p).BuffersI,Gen.Duration, ...
                ByPart.WaveFreq{p},ByPart.WaveForm{p},ByPart.AlphaRange{p},ByPart.isHarmonic{p},ByPart.RandPattern{p}, ...
        ...
        elseif ByPart.WhatVaries{p} == 'L'
            ...
            [B,A,Peaks,Phases] = subfun_preparesinanimation(Images(p).BufferBG,Images(p).BufferI,Gen.Duration, ...
                ByPart.WaveFreq{p},ByPart.WaveForm{p},range,ByPart.isHarmonic{p},0, 0, ByPart.SineWaveStartAngle{p});
        ...
        elseif ByPart.WhatVaries{p} == 'T'
            ...
            [B,A,Peaks,Phases] = subfun_preparesinanimation([],Images(p).BufferI,Gen.Duration, ...
                ByPart.WaveFreq{p},ByPart.WaveForm{p},ByPart.AlphaRange{p},ByPart.isHarmonic{p},0, 0, ByPart.SineWaveStartAngle{p});
...
function [BUFFERS,ALPHA,Peaks,Phases] = subfun_preparesinanimation(Buff1,Buff2,Dur,SineFreq,WaveForm,AlphaRange,isHarmonic,RandPattern,GapBetweenPatterns,SineWaveStartAngle)
...
% Sine Wave
%[ByFrame.Wave, ByFrame.Peaks, ByFrame.Phases, ByFrame.Slopes, ByRun.FinalFreq] = sinewave(Dur, SineFreq, getscreenfreq,AlphaRange, isHarmonic, SineWaveStartAngle,'plot');
[ByFrame.Wave, ByFrame.Peaks, ByFrame.Phases, ByFrame.Slopes, ByRun.FinalFreq] = generatewave(Dur, SineFreq, getscreenfreq,WaveForm,AlphaRange, isHarmonic, SineWaveStartAngle); % sinewave -> generatewave (G.V. 24/06/13)
