% SinStim_*   Parameter file for SinStim 1.6.1
...
% Gamma:
Gen.ScreenGamma = 1; % Screen gamma value. Must be measured for each screen.
...
Gen.Scale = [.95 1 1.1]; % Image scales. 1=100pc  <v1.3>
...
%% EXPERIMENT
% Durations:
Gen.DelayBefore = .2; % (sec) # secs before stim. begin. Use "rand" (e.g.: 5 + 10*rand;) to get random delay. <v1.1>
Gen.Duration = 2; % (sec) Duration of stimulation.
Gen.DelayAfter = .2; % (sec) <v1.1>
...
ByPart.WaveFreq = {1}; % Sinusoid freq. (Hz). NB: 4 Hz sinusoid = 8 identities per sec!
ByPart.isHarmonic = 0; % 1: Round to closest screen frequency divisor. 0: Don't.
%ByPart.BetweenRepeat = 1; % <v1.5: obsolete>

% Sinusoidal Luminance Variation:
% Luminance of images will be multiply a ratio sinusoidally oscillating between
% those two values. The max value can exceed 1 [not yet supported: use values <= 1.]
ByPart.AlphaRange = [0 1]; % [min,max] (ratios)
